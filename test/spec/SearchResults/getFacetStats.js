...
test('getFacetStats(facetName) returns stats for any facet or disjunctiveFacet', t => {
  const searchParams = new SearchParameters({
    facets: ['age', 'country'],
    disjunctiveFacets: ['price'],
  });
  const result = new SearchResults(searchParams, response.results);
...
test('getFacetStats(facetName) returns stats if the facet is both a regular and disjunctive facet', t => {
  const searchParams = new SearchParameters({
    facets: ['price'],
    disjunctiveFacets: ['price'],
  });
