...
test('Adding refinments should add an entry to the refinments attribute', t => {
  const facetName = 'facet1';
  const facetValue = '42';

  const helper = algoliasearchHelper(emptyClient, 'index', {
    facets: [facetName],
  });
  ...
  t.ok(
    _.size(helper.state.facetsRefinements) === 1,
  ...
  t.ok(
    _.size(helper.state.facetsRefinements) === 1,
    'when adding the same, should still be one'
  );
...
test('Adding several refinements for a single attribute should be handled', t => {
  const facetName = 'facet';

  const helper = algoliasearchHelper(emptyClient, null, {
    facets: [facetName],
  });
  ...
  t.ok(
    _.size(helper.state.facetsRefinements[facetName]) === 1,
    'Adding one refinement, should have one'
  );
  helper.addRefine(facetName, 'value2');
  t.ok(
    _.size(helper.state.facetsRefinements[facetName]) === 2,
    'Adding another refinement, should have two'
  );
  helper.addRefine(facetName, 'value1');
  t.ok(
    _.size(helper.state.facetsRefinements[facetName]) === 2,
    'Adding same refinement as the first, should have two'
  );

  t.end();
});

test('Toggling several refinements for a single attribute should be handled', t => {
  const facetName = 'facet';
  ...
  t.deepEqual(
    helper.state.facetsRefinements[facetName],
    ['value2'],
...
test('Using toggleRefine on a non specified facet should throw an exception', t => {
  const helper = algoliasearchHelper(emptyClient, null, {});
...
test('Removing several refinements for a single attribute should be handled', t => {
  const facetName = 'facet';
  ...
  t.ok(
    _.size(helper.state.facetsRefinements[facetName]) === 3,
    'Adding another refinement, should have two'
  );
  ...
  t.deepEqual(
    helper.state.facetsRefinements[facetName],
    ['value1', 'value3'],
    'should contain value1 and value3'
  );

  t.end();
});

test('isDisjunctiveRefined', t => {
  const facet = 'MyFacet';
  ...
  t.equal(
    helper.isDisjunctiveRefined(facet, value),
    false,
...
test('IsRefined should return true if the (facet, value ) is refined.', t => {
  const helper = algoliasearchHelper(emptyClient, null, {
    facets: ['facet1'],
  });
  ...
  t.equal(
    helper.isRefined('facet1', 'boom'),
    true,
    'the facet + value is refined >> true'
  );

  t.equal(
    helper.isRefined('facet1', 'booohh'),
    false,
    'value not refined but is a facet'
  );
  t.throws(
    _.bind(helper.isRefined, helper, 'notAFacet', 'maoooh'),
    'should throw as it is not a facet'
  );
...
test('isRefined(facet)/hasRefinements should return true if the facet is refined.', t => {
  const helper = algoliasearchHelper(emptyClient, null, {
    facets: ['facet1'],
  });

  t.equal(
    helper.isRefined('facet1'),
    false,
    'the facet is not refined yet >> false'
  );
  t.equal(
    helper.hasRefinements('facet1'),
    false,
    'the facet is not refined yet >> false'
  );
...
test('getRefinements should return all the refinements for a given facet', t => {
  const helper = algoliasearchHelper(emptyClient, null, {
    facets: ['facet1'],
...
test('getRefinements should return an empty array if the facet has no refinement', t => {
  const helper = algoliasearchHelper(emptyClient, null, {
    facets: ['facet1'],
...
test('[Conjunctive] Facets should be resilient to user attempt to use numbers', t => {
  const helper = algoliasearchHelper(emptyClient, null, {
    facets: ['facet1'],
    disjunctiveFacets: ['facet2'],
  });
  ...
  t.equal(
    helper.isRefined('facet1', 42),
    true,
    '[facet][number] should be refined'
  );
  t.equal(
    helper.isRefined('facet1', '42'),
    true,
    '[facet][string] should be refined'
  );

  const stateWithFacet1and42 = helper.state;

  helper.removeRefine('facet1', '42');
  t.equal(
    helper.isRefined('facet1', '42'),
    false,
    '[facet][string] should not be refined'
  );
  ...
  t.equal(
    helper.isRefined('facet1', 42),
    false,
    '[facet][number] should not be refined'
  );

  t.end();
});

test('[Disjunctive] Facets should be resilient to user attempt to use numbers', t => {
  const helper = algoliasearchHelper(emptyClient, null, {
    facets: ['facet1'],
    disjunctiveFacets: ['facet2'],
  });
  ...
  const stateWithFacet1Without42 = helper.state;
...
test('[Disjunctive] Facets should be resilient to user attempt to use numbers', t => {
  ...
  t.equal(
    helper.isDisjunctiveRefined('facet2', 42),
    true,
    '[facet][number] should be refined'
  );
  t.equal(
  ...
  const stateWithFacet2and42 = helper.state;
